...
const powers: Record<StateId, FrequencyPower> = {
  alpha: {
    ...
    benefits: ['May support sustained attention', 'Creates a calmer entry into study', 'Helps organize thoughts around one task'],
    uses: ['Studying', 'Reading', 'Planning', 'Morning clarity', 'Steady work'],
    technical: 'Left 180 Hz · Right 190 Hz · 10 Hz difference',
    guidance: 'Headphones recommended · Keep volume moderate'
  },
  gamma: {
    id: 'gamma',
    frequency: 'Gamma',
    ...
    benefits: ['May support high-demand concentration', 'Encourages active information processing', 'Useful for precise analytical sessions'],
    uses: ['Research', 'Programming', 'Complex study', 'Problem solving', 'Deep work'],
    technical: 'Left 220 Hz · Right 260 Hz · 40 Hz difference',
    guidance: 'Headphones recommended · Begin at a low volume'
  },
  theta: {
    id: 'theta',
    frequency: 'Theta',
    ...
    benefits: ['May support creative ideation', 'Encourages reflective thinking', 'Creates room for mental flexibility'],
    uses: ['Writing', 'Brainstorming', 'Meditation', 'Journaling', 'Visualization'],
    technical: 'Left 95 Hz · Right 99 Hz · 4 Hz difference',
    guidance: 'Headphones recommended · Best in a quiet setting'
  },
  delta: {
    id: 'delta',
    frequency: 'Delta',
    ...
    benefits: ['May support progressive relaxation', 'Helps reduce stimulation before rest', 'Creates a deliberate transition into stillness'],
    ...
    guidance: 'Headphones recommended · Use only when resting safely'
  },
  abundance: {
    id: 'abundance',
    frequency: 'Pure Tone',
    ...
    guidance: 'Speakers or headphones · Start at a very low volume'
  }
};
    ...
    <div className={`evFrequencyPowerBackdrop ${active.id} ${closing ? 'closing' : ''}`} role="presentation">
      <article className={`evFrequencyPowerCard ${active.id}`} role="dialog" aria-modal="true" aria-labelledby="ev-frequency-power-title">
        ...
        <div className="evFrequencyPowerGrid">
          <section>
            <h3>What this state may support</h3>
            <ul>{active.benefits.map((benefit) => <li key={benefit}>{benefit}</li>)}</ul>
          </section>
          <section>
            <h3>Best used for</h3>
            <div className="evFrequencyUseChips">{active.uses.map((use) => <span key={use}>{use}</span>)}</div>
          </section>
        </div>
